...
...in terms of values
```{r}
tabl(df, vars = "gender", labs.on = FALSE) # labs.on = FALSE is default
```
...or in terms of labels
```{r}
tabl(df, vars = "gender", labs.on = TRUE) # labs.on = TRUE is NOT the default
```
...in proportions
```{r}
tabl(df, vars = c("gender", "edu"), labs.on = TRUE, prop.digits = 3)
```
...cross-tab style
```{r}
head(tabl(df, vars = c("raceth", "edu"), labs.on = TRUE, wide.col = "gender"), 20)
```
...with non-value-labeled data.frames
```{r}
tabl(iris, "Species") # explicit vars arg with one-var ("Species")

tabl(mtcars, zero.rm = TRUE) # Note: vars arg null
```

For an alternative implementation of `tabl()` that always returns tabulations in 
terms of value labels (where they exist), see `tab_lab()` (with alias `tabl2()`). 
