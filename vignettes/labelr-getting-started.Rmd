...
## A Whirlwind Tour
labelr is an R package that supports creation and use of multiple types of 
labels for data.frames and data.frame variables (columns). This vignette 
provides an ad hoc introduction to core and ancillary labelr functionalities and 
uses cases. 
...
options are to accept the default "NA" label or convert these values to 
something else before labeling. The reasoning is that value labels are rarely 
appropriate for the types of variables and scenarios where you absolutely need 
to preserve the nuances of exotic values like -Inf and NaN. 
