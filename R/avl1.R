...
#'
#' @details
#' `avl1` is an alias for `add_val1()`. See the latter's documentation for
#' additional details.
#'
...
#' another label.
#' @param max.unique.vals `avl1`() will not assign value labels to non-
...
#' df <- mtcars
#' # add value labels
#' # first, using add_val_labs() -- avl1() example is below
#' df <- add_val_labs(
#'   data = df,
#'   vars = "carb", # note, vars arg; avl1() takes var arg
#'   vals = c(1, 2, 3, 4, 6, 8),
#'   labs = c(
#'     "1-carb", "2-carbs",
#'     "3-carbs", "4-carbs",
...
#' )
#'
#' # now, using avl1(), where single var arg can be unquoted (cyl, not "cyl")
...
#' )
#'
avl1 <- function(data, var, vals, labs,
                 max.unique.vals = 10,
                 init = FALSE) {
  # check max vals
...
Adjust max.unique.vals arg?", var
    ...
    if (length(final_names) != length(final_vals)) {
      stop(sprintf(
        "\nConcerning Var --%s--
add_val1() is for one-to-one value labeling and does not support applications
...
variable and consider using, e.g., add1m1() or add_quant1(), which do
