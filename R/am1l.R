#' (Alias for) `add_m1_lab()`
#'
#' @description
#' `am1l()` is an alias for `add_m1_lab()`.
#'
#' @details
#' See the entry for `add_m1_lab` for further details.
#'
...
#' and partial=TRUE, then `am1l` will attempt to label not only "x1",
...
#' tantamount to telling am1l() that you actually do not wish to label
#' any of the variables that you specified in vars, resulting in no variables
#' receiving value labels.
#' @param max.unique.vals `am1l`() will not assign value labels to non-
...
am1l <- function(data, vars, vals, lab,
