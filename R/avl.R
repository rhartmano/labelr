#' (Alias for) `add_val_labs()`
#'
#' @description
#' Add variable value-specific, descriptive value labels to a data.frame.
#'
#' @details
#' `avl` is an alias for `add_val_labs()`. See the latter's documentation for
#' additional details.
#'
...
#' and partial=TRUE, then `avl` will attempt to label not only "x1",
...
#' tantamount to telling avl() that you actually do not wish to label
#' any of the variables that you specified in vars, resulting in no variables
#' receiving value labels.
#' @param max.unique.vals `avl`() will not assign value labels to non-
...
#' # note that this is not avl(); add_val1() has "var" (not "vars" arg)
#' df <- add_val1(
#'   data = df,
...
#' # note, by default, avl() "vars" arg will do partial matching
...
#' head(dflik_conv, 3)
avl <- function(data, vars, vals, labs,
...
Adjust max.unique.vals arg?", var
    ...
    final_names <- unique(names(get_labs_att(data_unique, this_var_val_label)[[1]]))
    final_vals <- unique(unname(get_labs_att(data_unique, this_var_val_label)[[1]]))

    if (length(final_names) != length(final_vals)) {
      stop(sprintf(
        "\nConcerning Var --%s--
avl() is for one-to-one value labeling and does not support applications
where a single value label is applied to multiple distinct values of a variable.

Use get_val_labs() to see which value labels are currently applied to this
variable and consider using, e.g., add_m1_lab() or add_quant_labs(), which do
support some one-label-to-many-values applications.",
        var
      ))
    }
