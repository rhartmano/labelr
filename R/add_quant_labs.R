...
#' and partial=TRUE, then `add_quant_labs` will attempt to label not only "x1",
...
#' head(df_labson)
add_quant_labs <- function(data, vars, qtiles = NULL, vals = NULL, labs = NULL,
...
\nNote: labelr is not optimized for data.frames this large.")
  ...
  for (i in seq_along((vars))) {
    ...
    # remove NA and irregular values for labeling purposes
