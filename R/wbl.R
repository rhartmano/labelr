#' (Alias for) `with_both_labs()`
#'
#' @description
#' `wbl` instructs R function calls to swap in variable name labels
#' for column names AND variable value labels for variable values in the objects
#' they return or side effects they produce.
#'
#' @details
#' `wbl` is an alias for `with_both_labs()`. See the latter's documentation for
#' additional details.
#'
...
wbl <- function(data, ...) {
  tryCatch(
    ...
    error = function(e) {
      message('
Error: Invalid argument or syntax passed to wbl(). \n

wbl() is for simple, one-step descriptive/exploratory data analysis calls, not
